...
const halveCorrectly = (a, b, fundamentalEdges, newFundamentalEdges) => {
    const [ax, ay, az, af] = a;
    const [bx, by, bz, bf] = b;
    // midpoint is in the FD if edges are
    const f = fundamentalEdges.has(edgeKey(a, b));
    const v = [(ax + bx) / 2, (ay + by) / 2, (az + bz) / 2, f];
...
const edgeKey = ([ax, ay, az, af], [bx, by, bz, bf]) => [[ax, ay, az].toString(), [bx, by, bz].toString()].sort().toString();
const lerp = (a, b, factor) => a * (1 - factor) + b * factor;
const vLerp = ([ax, ay, az, af], [bx, by, bz, bf], factor) => [lerp(ax, bx, factor), lerp(ay, by, factor), lerp(az, bz, factor), af && bf]; // placeholder _f due to bad design
...
export class Mesh {
    constructor(rawVertices, rawFaces, withFundamentalDomain) {
        ...
        // it's so jank fr
        const extraFlatVertices = [];
        const extraFundamentalDomain = [];
        const extraNormals = [];
    ...
    subdivide(exponent) {
        ...
        let newTriangles = [];
        let newNormals = [];
        let newFundamentalEdges = new Set();
        for (let i = 0; i < this.triangles.length; i += 3) {
